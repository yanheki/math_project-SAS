/*$ removed coz cannot run Boxcox transformation*/
data q1;
...
proc print data=q1;
run;
/* select categorical vairable*/
proc glmselect data=q1;
	class south sex marr ;
	model wage= south sex marr  education experience;
run;
/*deleted sex*education after concerning p-value>0.05*/
proc glm data=q1;
class sex;
model wage= sex education experience  sex*experience/ ss3 solution;
run;
/*test residual is normal or not*/
proc glm data=q1 PLOTS=(DIAGNOSTICS RESIDUALS); /* Create the residual plots as PROC REG */
class sex;
model wage= sex education experience  sex*experience;
output  OUT=stat P=pred R=resid RSTUDENT=ti STUDENT=ri cookd=cookd dffits=dffits;

run;
proc print data=stat;
run;

proc univariate data=stat normal;
var ti;
LABEL ti='Studentized residual';
PROBPLOT ti /* QQPLOT ti */ / NORMAL(MU=EST SIGMA=EST COLOR=YELLOW W=1) SQUARE;
run;

/*use boxcox to transform to normal*/
PROC TRANSREG DATA=q1 PBOXCOXTABLE;
	MODEL BOXCOX (wage / LAMBDA = -2 to 2 by 0.5) = IDENTITY(sex education experience  sex*experience);
	OUTPUT OUT=tran LEVERAGE RESIDUALS P;
RUN;

proc print data=tran;
run;
/*use Transformed model to test normal*/
proc glm data=tran;
class sex;
model Twage= sex education experience  sex*experience;
output  OUT=stat P=pred R=resid RSTUDENT=ti STUDENT=ri;
run;
proc print data=stat;
run;
...
/*test homogeneity of variance of residual----------------------->>>>>>>PROBLEM!!! CANNOT USE MEANS. MEANS ONLY FOR ONEWAY!!*/

PROC GLM DATA=tran PLOTS=NONE;
	CLASS sex;
	MODEL  Twage= sex education experience  sex*experience;
	MEANS sex education experience  sex*experience/ HOVTEST=BARTLETT 
				  HOVTEST=LEVENE (TYPE=SQUARE) 
				  HOVTEST=BF 
				  HOVTEST=OBRIEN (W=0.5) WELCH;
RUN;
...
proc glm data = q1;
  class sex race marr occupation ;
  model wage =    race occupation sex |marr   /ss3 ;
run;


/* seperate the data set in to m0 and f1*/
...
/* correct?*/
proc robustreg data=m0;
   class  sex race marr occupation ;
  model wage =  race marr occupation/ diagnostics;
   
   output out=robout r=resid sr=stdres;
run;
/* how to remove outlier??  output correect?*/
/* For m0*/
proc glm data=m0 ;
class sex race marr occupation ;
  model wage =  race marr occupation ;

  output out=stat p=p R=e rstudent=t student=r cookd=cookd dffits = dffits ;
run;
quit;
...
var e;
run;
/* class var??  adjust = bon e tdiff pdiff  ??*/
proc glm data=q1 plots = none;
class sex race marr occupation ;
model wage =  sex*marr  occupation sex*marr*occupation race*marr*occupation ;
lsmeans sex race marr occupation / hovtest = bf;
run;

/* done??*/
/* interputation of the result:
wage =  sex*marr  occupation sex*marr*occupation race*marr*occupation  */
